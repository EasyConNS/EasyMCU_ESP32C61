...
- **NS2 BLE 协议栈实现**：完整实现了 NS2 手柄的配对、认证、数据加密等流程
- **模块化架构**：
...
### 蓝牙协议栈要求
本项目基于 **ESP-IDF** 框架开发。理论上，任何使用 Apache NimBLE 开源堆栈的 ESP-IDF 固件都可以支持。但由于 NS2 协议突破了 BLE 最低连接间隔的规范（标准最低为 7.5ms，NS2 要求 5ms），需要对 NimBLE 协议栈进行修改。
...
### 补丁来源
NS2要求蓝牙连接间隔为 5ms，该数值低于BLE标准规范规定的 7.5ms 最低限值。本项目仅用于科研与教学研究，探究蓝牙 5.0 性能极限以及与非标设备的互通兼容性。若要在 ESP32-C6/C61 芯片平台实现该连接间隔，需修改 ESP-IDF 框架中 NimBLE 蓝牙协议栈的固定参数常量。
